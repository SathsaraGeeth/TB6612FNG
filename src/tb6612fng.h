...
void __attribute__((weak)) start_pwm(pin_t pin, int frequency, double duty_cycle);
void __attribute__((weak)) noop(int time);

/* main data structure */

typedef struct {
    double DUTY_CYCLE; // 0-1 the duty cycle of pwm signal
    int FPWM; // the frequency of the pwm signal
